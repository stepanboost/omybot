...
2) Формулы и выражения — используй Unicode математические символы для красивого отображения.
...
- Степени: x², xⁿ⁺¹, x^(n+1)
- Индексы: x₁, xₘₐₓ, x_max
- Дроби: (a)/(b) или a/b
- Корни: √x, ³√x, √(a+b)
- Греческие буквы: α, β, γ, θ, π, Δ
- Функции: sin, cos, tan, arctan, ln, log
- Стрелки: →, ←, ⇒, ⇔
- Символы: ≈, ≠, ≤, ≥, ±, ∞
...
ВАЖНО: Используй Unicode символы для красивого отображения формул в Telegram. НЕ используй LaTeX блоки.

Тон: чёткий, дружелюбный, без морализаторства. Язык — русский."""
        
        if subject_hint:
            subject_prompts = {
                "математика": "\n\nМатематика: используй Unicode символы для формул. Примеры: x² + 2x + 1 = 0, √(a² + b²), sin(α), πr²",
                "физика": "\n\nФизика: используй Unicode символы для формул. Примеры: F = ma, E = mc², v = v₀ + at, h = v₀²/(2g)",
                "химия": "\n\nХимия: используй Unicode символы для формул. Примеры: H₂O, CO₂, 2H₂ + O₂ → 2H₂O, CaCO₃ + 2HCl → CaCl₂ + H₂O + CO₂↑",
    ...
    def _parse_response(self, content: str, subject_hint: str = None) -> Dict[str, Any]:
        ...
        # Ищем математические формулы с Unicode символами
        # Ищем формулы с греческими буквами, степенями, корнями
        math_patterns = [
            r'[αβγθπΔ][\w\s\+\-\*\/\^\(\)\d\.]+',  # Греческие буквы
            r'[a-zA-Z]²|[a-zA-Z]³|[a-zA-Z]ⁿ',  # Степени
            r'√[a-zA-Z0-9\(\)\+\-\*\/]+',  # Корни
            r'[a-zA-Z]₁|[a-zA-Z]₂|[a-zA-Z]₃',  # Индексы
            r'[a-zA-Z]₂O|[a-zA-Z]₃|[a-zA-Z]₄',  # Химические формулы
        ]
